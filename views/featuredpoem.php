...
<script type="text/javascript">
        ...
        var starNumber;	// Isthe maximum number of stars
        var clicked; // Is the holding pattern for clicked state
        var preValue; // Is the preValue value onces a selection has been made
        var rated;
        ...
        function notHover(me) {
            if (!rated) {
                if (!preValue) {
                    for (i = 1; i <= starNumber; i++) {
                        document.getElementById("_" + i).className = "";
                        document.getElementById("Status").innerHTML = me.parentNode.title;
                    }
                } else {
                    rating(preValue);
        ...
        function rateThisPoem(me) {
            if (!rated) {
                document.getElementById("Status").innerHTML = document.getElementById("Saved").innerHTML + " :: " + me.title;
                preValue = me;
                rated = 1;
                recordRate(me);
                rating(me);
        ...
        function recordRate(rate) {
            ...
            form.submit();
            alert("Your have rated " + rate.title + " With score of " + rate.name);
        }
